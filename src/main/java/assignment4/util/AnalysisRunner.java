...
//		new EpisodeSequenceVisualizer(v, domain, Arrays.asList(ea));
	...
	public void runQLearning(BasicGridWorld gen, Domain domain,
		...
		EpisodeAnalysis ea = null;
		int increment = MAX_ITERATIONS/NUM_INTERVALS;
		RandomPolicy rpol = new RandomPolicy(domain);
//		EpsilonGreedy egpol = new EpsilonGreedy(0.2);
		for(int numIterations = increment;numIterations<=MAX_ITERATIONS;numIterations+=increment ){
...
//			BoltzmannQPolicy bpol = new BoltzmannQPolicy(agent, 0.6);
			EpsilonGreedy egpol = new EpsilonGreedy(agent, 0.2);
			agent.setLearningPolicy(egpol);
